...
class textDraw:
    ...
    @micropython.native
    def drawChar(self, c, buffer, offset, reverse):
        l = len(buffer)
        f = self.FONT[ord(c)-ord(' ')]
        for col in f:
            if reverse:
                buffer[offset] = self.flipBits(col)
                offset -= 1
            else:
                buffer[offset] = col
                offset += 1
    ...
    @micropython.native
    def measureString(self, message):
    ...
    @micropython.native
    def drawString(self, message, buffer=None, offset=0, reverse=False):
        # Measure message length in columns
        width = self.measureString(message)
        ...
        for m in message:
            offset = self.drawChar(m, buffer, offset, reverse)
